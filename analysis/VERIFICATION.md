...
- `00_map_exposure_rsids.py` streams each selected meta_EUR source once, retains the `P<5×10⁻⁸` mapping superset, verifies the frozen dbSNP build-157 MD5, queries indexed GRCh38 positions in bounded trait batches and assigns rsIDs only after exact unordered allele-pair matching.
- A real-data smoke test scanned all 103,927,003 rows for `Cholesterol_in_very_large_HDL` and generated 109,745 mapping records: 106,178 `matched`, 1,098 `ambiguous`, 2,448 `no_allele_match` and 21 `no_dbsnp_entry`. Variant IDs, rsIDs and statuses matched the frozen mapping table exactly, with zero discrepancies.
- The dependency-free synthetic test covers direct and reversed allele orientation, multiple rsIDs, allele mismatch, absent dbSNP positions, invalid alleles and significance filtering. The analysis-level preflight now rejects empty maps, duplicate variant IDs, unsupported status values and invalid uniquely matched rsIDs.
- The 249-trait full mapping was not rerun during release preparation. It remains an explicitly user-run real-data stage because the source files and 29.5-GB dbSNP archive are not distributed. The one-trait smoke is required before full execution, and the emitted QC and run manifest retain the source-content and output hashes needed for comparison.
- Released aggregate tables, figures and scientific results are unchanged from v0.2.17. The repository audit, Python compilation, source-preflight test, rsID-mapping test, deterministic figure generation, classification replay and synthetic MR/colocalization tests must pass in the release commit and its clean GitHub Actions run.
